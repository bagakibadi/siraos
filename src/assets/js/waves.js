...
(function (window, factory) {
  ...
  function getWavesElements(nodes) {
    ...
    if (stringRepr === '[object String]') {
      return $$(nodes);
    } if (isObject(nodes) && /^\[object (Array|HTMLCollection|NodeList|Object)\]$/.test(stringRepr) && nodes.hasOwnProperty('length')) {
      return nodes;
    } if (isDOMNode(nodes)) {
      return [nodes];
    }
  ...
  const TagWrapper = {
    ...
    input(element) {
      ...
      const { backgroundColor } = elementStyle;
  ...
  Waves.ripple = function (elements, options) {
    ...
    if (elementsLen) {
      let element; let pos; let off; const centre = {}; let
        i = 0;
      const mousedown = {
        type: 'mousedown',
        button: 1,
